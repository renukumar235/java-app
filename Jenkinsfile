pipeline {
    ...
    tools {
        // Install the Maven version configured as "M3.9" and add it to the path.
        maven "maven3.9"
    }
    ...
    stages {
        stage('checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/dev']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/renukumar235/java-app.git']])
            }
        }
        stage('build') {
            steps {
